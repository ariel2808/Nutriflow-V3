...
import { ChevronLeft, Search, Camera, Plus, Check } from 'lucide-react';
...
export function CreateMealScreen({ onBack, onSave }: CreateMealScreenProps) {
  ...
  const handleSave = () => {
    if (canSaveMeal(mealName, selectedIngredients)) {
      const mealData = createMealData(mealName, selectedIngredients, totalNutrition);
      onSave(mealData);
    }
  };
